  ...
  if (profile?.role !== 'teacher' && profile?.role !== 'admin') {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-white p-6">
        ...
        <p className="text-white/60">Bu sahifa faqat teacher va admin uchun.</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-8 pb-12">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-5">
        <div>
          <div className="flex items-center gap-2 mb-2 text-purple-400">
            ...
            <span className="text-xs font-black uppercase tracking-[0.25em]">Teacher panel</span>
          </div>
          <h1 className="text-3xl font-black text-white tracking-tight">Talabalar nazorati</h1>
          <p className="text-white/50 mt-1">Testlar, amaliyotlar va guruhlar bo'yicha umumiy holat.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-4">
        {[
          { label: 'Jami talabalar', value: stats.totalStudents, icon: Users, color: 'text-cyan-300', bg: 'bg-cyan-500/10' },
          { label: 'Guruhlar', value: stats.activeGroups, icon: GraduationCap, color: 'text-purple-300', bg: 'bg-purple-500/10' },
          { label: 'Test natijalari', value: stats.completedTests, icon: CheckCircle2, color: 'text-green-300', bg: 'bg-green-500/10' },
          { label: 'Amaliyotlar', value: stats.completedPractices, icon: Target, color: 'text-pink-300', bg: 'bg-pink-500/10' },
          { label: "O'rtacha", value: `${stats.avgScore}%`, icon: BarChart3, color: 'text-blue-300', bg: 'bg-blue-500/10' },
        ].map((stat, index) => (
          <motion.div
            key={stat.label}
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-lg font-bold text-white">Baholash</h2>
            <p className="text-white/50 text-sm">
              {pendingSubmissionsCount > 0
                ? `${pendingSubmissionsCount} ta kutilayotgan topshiriq bor`
                : "Hozircha kutilayotgan topshiriq yo'q"}
            </p>
          </div>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_420px] gap-6">
        <div className="glass-panel overflow-hidden border-white/5">
          <div className="p-6 border-b border-white/5 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h2 className="text-xl font-bold text-white">Talabalar ro'yxati</h2>
              <p className="text-white/40 text-sm mt-1">Har bir talabaning umumiy o'zlashtirish holati.</p>
            </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-white/[0.02] text-white/40 text-xs font-bold uppercase tracking-wider">
                  ...
                  <th className="px-6 py-4">Amaliyot</th>
                  <th className="px-6 py-4">O'rtacha</th>
              ...
              <tbody className="divide-y divide-white/5">
                {loading ? (
                  <tr>
                    <td colSpan={6} className="px-6 py-12 text-center text-white/30">Yuklanmoqda...</td>
                  </tr>
                ) : studentRows.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-6 py-12 text-center text-white/30">Talabalar topilmadi</td>
                  </tr>
                ) : (
                  studentRows.map((row) => (
                    <tr key={row.student.id} className="hover:bg-white/[0.02] transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="text-white font-bold text-sm">{row.student.full_name || 'Talaba'}</div>
                            <div className="text-white/35 text-xs">@{row.student.username || 'login yoq'}</div>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-white/35 text-xs">
                        {row.lastActivity ? new Date(row.lastActivity).toLocaleString() : 'Hali yoq'}
                      </td>
                    </tr>
                  ))
                )}
        ...
        <div className="glass-panel overflow-hidden border-white/5">
          ...
          <div className="divide-y divide-white/5">
            {loading ? (
              ...
              <div className="p-10 text-center text-white/30">Hozircha natija yo'q</div>
            ) : (
              latestResults.map((result) => (
                <div key={result.id} className="p-5 hover:bg-white/[0.02] transition-colors">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <p className="text-white font-bold text-sm">{result.profiles?.full_name || 'Talaba'}</p>
                      <p className="text-white/35 text-xs mt-1">{result.profiles?.group_name || 'Guruhsiz'}</p>
                    </div>
